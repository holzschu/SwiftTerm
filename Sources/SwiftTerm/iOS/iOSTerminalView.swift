...
open class TerminalView: UIScrollView, UITextInputTraits, UIKeyInput, UIScrollViewDelegate, TerminalDelegate, UIPointerInteractionDelegate {
    ...
    */
    ...
    public override var accessibilityElements: [Any]? {
        // Copied from WWDC 21 "Bring accessibility charts to your apps", but is not working.
        // Code is actually called, but doesn't work.
        get {
            var returnElements = [UIAccessibilityElement]()
            let pageHeight = max(bounds.height, cellDimension.height)
            let lines = Int(floor(pageHeight/cellDimension.height))
            let startLine = Int(floor(contentOffset.y / cellDimension.height))
            for line in startLine...startLine + lines {
                let start = Position(col: 0, row: line)
                let end = Position(col: terminal.buffer.lines[line].count, row: line)
        ...
        set {
            NSLog("accessibilityElements::set has been called")
        }
    }
